    ...
    foreach pp $Prepost {
      ...
      switch -exact -- $a {
        ...
        -lbxsel {
          ...
          if {$v>=0} {
puts "$w selection set $v;  $w yview $v;  $w activate $v"  ;#! TODEL
            $w selection set $v
            $w yview $v
            $w activate $v
          }
...
puts "$w tag configure sel -borderwidth 1"  ;#! TODEL
...
puts "$w add \[$wid $wt $Attrs\] $attr"  ;#! TODEL
...
puts "pack propagate $w 0"  ;#! TODEL
            pack propagate $w 0
          } else {
puts "grid propagate $w 0"  ;#! TODEL
            grid propagate $w 0
          }
        }
        -columnoptions {
          foreach {col opts} $v {
puts "$w column $col $opts"  ;#! TODEL
            $w column $col {*}$opts
          }
        }
        -selborderwidth {
puts "$w tag configure sel -borderwidth $v"  ;#! TODEL
...
set attrtmp [string map [list \{\{ \{ \}\} \}] $attrs]    ;#! TODEL
puts "$widget $wname $attrtmp"  ;#! TODEL
...
puts "pack forget [string range $opts 6 end]"  ;#! TODEL
          } else {
            pack $wname {*}$opts
puts "pack $wname $opts"  ;#! TODEL
          }
        } else {
puts "grid $wname -row $row -column $col -rowspan $rowspan -columnspan $colspan -padx 1 -pady 1 $options"  ;#! TODEL
...
puts "pack \[frame $w -background [lindex [my csGet] 3]\] -expand 1 -fill both"  ;#! TODEL
    }
puts "wm title $wtop {$ttl}"  ;#! TODEL
    wm title $wtop $ttl
    return $wtop
  }
  #_______________________
